'use client'

import React, { useState } from 'react'
import ContactForm from './ContactForm'

const TabContent1 = () => (
  <div className="p-4">
    <h2 className="text-xl font-bold mb-2">First Tab Content</h2>
    <p>This is the content for the first tab. You can put anything here.</p>
  </div>
)

const TabContent2 = () => (
  <div className="p-4">
    <h2 className="text-xl font-bold mb-2">Second Tab Content</h2>
    <p>
      This is the content for the second tab. It can be completely different.
    </p>
  </div>
)

const TabContent3 = () => (
  <div className="p-4">
    <h2 className="text-xl font-bold mb-2">Third Tab Content</h2>
    <p>
      Here&apos;s the content for the third tab. Each tab can have unique
      content.
    </p>
  </div>
)

const tabs = [
  { title: 'Form', component: ContactForm },
  { title: 'Offices', component: TabContent2 },
  { title: 'FAQs', component: TabContent3 },
]

const ContactMethods = () => {
  const [activeTab, setActiveTab] = useState(0)

  const ActiveComponent = tabs[activeTab].component

  return (
    <section className="py-32 grid grid-cols-1 md:grid-cols-3 gap-6">
      <div className="tabs w-full md:col-span-2 flex flex-col gap-4">
        
        <div className="tab-buttons w-full px-6 flex gap-6 md:gap-12 border-b border-black border-opacity-25">
          {tabs.map((tab, index) => (
            <button
              key={index}
              onClick={() => setActiveTab(index)}
              className={`border-b-2 px-4 ${activeTab === index ? 'text-indigo opacity-100 border-indigo' : 'text-foreground opacity-60 border-transparent hover:border-indigo transition-all duration-300'}`}
            >
              {tab.title}
            </button>
          ))}
        </div>

        
        <div className="w-full">
          <ActiveComponent />
        </div>
      </div>

      <div className="info-hub w-full md:col-span-1"></div>
    </section>
  )
}

export default ContactMethods
